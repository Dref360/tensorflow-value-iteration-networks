...
if (config.untied_weights):
    logits, nn, r = VI_Untied_Block(X, S1, S2, config)
else:
    logits, nn, r = VI_Block(X, S1, S2, config)
...
with tf.Session() as sess:
    ...
    for epoch in range(int(config.epochs)):
        ...
        for i in (range(0, Xtrain.shape[0], batch_size)):
            ...
            if j <= Xtrain.shape[0]:
                ...
                _, e_, c_, r_ = sess.run([optimizer, err, cost, r], feed_dict=fd)
                avg_err += e_
                avg_cost += c_
        ...
        if config.log:
            ...
            summary.value.add(tag='Average error {} {} {} BatchNorm'.format(sd, config.untied_weights, config.k),
                              simple_value=float(avg_err / num_batches))
            summary.value.add(tag='Test Accuracy {} {} {} BatchNorm'.format(sd, config.untied_weights, config.k), simple_value=float(100 * (1 - acc)))
            summary.value.add(tag='Average cost {} {} {} BatchNorm'.format(sd, config.untied_weights, config.k),
    ...
    saver.save(sess, 'my-model{}{}batchnorm'.format(sd, 'untied' if config.untied_weights else ''))
    print("Model Saved!")